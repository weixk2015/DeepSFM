...
class disparityregression_p(nn.Module):
    def __init__(self, nlabel, std=0.2):
        ...
        disp = Variable(torch.Tensor(np.array(range(int(-nlabel / 12), int(nlabel / 12)))).cuda() + 0.5, requires_grad=False)
...
class disparityregression_p1(nn.Module):
    def __init__(self, nlabel, std=0.2):
        ...
        disp = Variable(torch.Tensor(np.array(range(int(-nlabel / 2), int(nlabel / 2)))).cuda() + 0.5, requires_grad=False)
        self.disp = -disp / disp[0] * std * 2.5
        #self.disp = Variable(torch.Tensor(np.reshape(np.array(range(maxdisp)),[1,maxdisp,1,1])).cuda(), requires_grad=False)
...
class PoseRegression(nn.Module):
    def __init__(self, nlabel, std=0.2):
        ...
        self.nlabel = nlabel
        trans = Variable(torch.Tensor(np.array(range(int(-nlabel / 2), int(nlabel / 2)))).cuda() + 0.5, requires_grad=False)
